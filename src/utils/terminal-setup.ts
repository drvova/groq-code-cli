...
export async function runTerminalSetup(): Promise<SetupResult> {
	...
	try {
		switch (type) {
			...
			case 'kitty':
				return await setupKitty();
			case 'alacritty':
				return await setupAlacritty();
			case 'hyper':
				return await setupHyper();
			case 'windows-terminal':
				return await setupWindowsTerminal();
			default:
				return {
					success: false,
...
async function setupBashCompletions(
...
async function setupZshCompletions(
	terminal: TerminalType,
): Promise<SetupResult> {
...
async function setupFishCompletions(
	terminal: TerminalType,
): Promise<SetupResult> {
...
/**
 * Setup Ghostty terminal integration
 */
async function setupGhostty(): Promise<SetupResult> {
	...
	try {
		...
		return {
			success: true,
			message: 'Ghostty setup completed',
			details: 'Configuration file detected, GPU acceleration enabled',
		};
	} catch {
		return {
			success: true,
			message: 'Ghostty detected',
			details: 'Create ~/.config/ghostty/config for custom configuration',
		};
	}
}

/**
 * Setup Kitty terminal integration
 */
async function setupKitty(): Promise<SetupResult> {
	const configPath = path.join(
		process.env.HOME || '',
		'.config',
		'kitty',
	...
	try {
		...
		return {
			success: true,
			message: 'Kitty setup completed',
			details: 'Configuration detected, image protocol available',
		};
	} catch {
		return {
			success: true,
			message: 'Kitty detected',
			details: 'Create ~/.config/kitty/kitty.conf for custom configuration',
		};
	}
}

/**
 * Setup Alacritty terminal integration
 */
async function setupAlacritty(): Promise<SetupResult> {
	const configPath = path.join(
		process.env.HOME || '',
		'.config',
		'alacritty',
	...
	const yamlConfigPath = path.join(
		process.env.HOME || '',
		'.config',
		'alacritty',
		'alacritty.yml',
	...
	try {
		...
		return {
			success: true,
			message: 'Alacritty setup completed',
			details: 'Configuration detected, GPU acceleration enabled',
		};
	} catch {
		return {
			success: true,
			message: 'Alacritty detected',
			details:
				'Create ~/.config/alacritty/alacritty.toml for custom configuration',
		};
	}
}

/**
 * Setup Hyper terminal integration
 */
async function setupHyper(): Promise<SetupResult> {
	const configPath = path.join(process.env.HOME || '', '.hyper.js');

	try {
		await fs.access(configPath);
		return {
			success: true,
			message: 'Hyper setup completed',
			details: 'Configuration detected, plugin system available',
		};
	} catch {
		return {
			success: true,
			message: 'Hyper detected',
			details: 'Create ~/.hyper.js for custom configuration and plugins',
		};
	}
}
 ...
 */
async function setupWindowsTerminal(): Promise<SetupResult> {
	const configPath = path.join(
	...
	try {
		...
		return {
			success: true,
			message: 'Windows Terminal setup completed',
			details: 'Configuration detected, multi-profile support enabled',
		};
	} catch {
		return {
			success: true,
			message: 'Windows Terminal detected',
			details: 'Use Ctrl+, to open settings and configure profiles',
		};
	}
}
