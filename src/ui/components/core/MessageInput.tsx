...
export default function MessageInput({
	...
	useInput((input, key) => {
		...
		if (key.upArrow) {
			if (showSlashCommands) {
			...
			} else if (value.length === 0 && userMessageHistory.length > 0) {
				// Navigate to message history only when input is empty
				if (historyIndex === -1) {
				...
				// Move cursor to start when there's text
				setCursorPosition(0);
			}
			return;
		}
		...
		if (key.downArrow) {
			if (showSlashCommands) {
				...
				// Move cursor to end when there's text
				setCursorPosition(value.length);
			}
			return;
		}
