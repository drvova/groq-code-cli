...
export class ToolRegistry {
	private static schemas: Map<string, ToolSchema> = new Map();
	private static toolCategories: Map<string, ToolCategory> = new Map();
	...
	static initialize(): void {
		...
		for (const schema of BUILT_IN_TOOL_SCHEMAS) {
			this.schemas.set(schema.function.name, schema);
		}

		// Set built-in tool categories
		this.toolCategories.set('read_file', 'safe');
		this.toolCategories.set('list_files', 'safe');
		this.toolCategories.set('search_files', 'safe');
		this.toolCategories.set('create_tasks', 'safe');
		this.toolCategories.set('update_tasks', 'safe');
		this.toolCategories.set('create_file', 'approval_required');
		this.toolCategories.set('edit_file', 'approval_required');
		this.toolCategories.set('delete_file', 'dangerous');
		this.toolCategories.set('execute_command', 'dangerous');
	...
	static registerTool(
		schema: ToolSchema,
		category: ToolCategory = 'approval_required',
	): void {
		this.schemas.set(schema.function.name, schema);
		this.toolCategories.set(schema.function.name, category);
	}
	...
	static getToolCategory(toolName: string): ToolCategory | undefined {
		this.initialize();
...
export const ALL_TOOL_SCHEMAS = BUILT_IN_TOOL_SCHEMAS;
